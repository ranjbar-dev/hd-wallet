...
// --- WS-A: curve verifier negative paths (verify* were 73-79% covered) ---

func TestCurveVerifyNegativePaths(t *testing.T) {
	msg := []byte("message")
	identity := make([]byte, 32) // [0x01,0...0] is the valid ed25519 identity encoding
	identity[0] = 0x01
	nonCanonical := bytes.Repeat([]byte{0xff}, 32) // rejected by edwards25519 SetBytes / SetCanonicalBytes

	t.Run("cardano", func(t *testing.T) {
		if verifyCardano(make([]byte, 31), msg, make([]byte, 64)) {
			t.Error("short pubkey should fail")
		}
		if verifyCardano(nonCanonical, msg, make([]byte, 64)) {
			t.Error("non-canonical point should fail")
		}
		// valid point, non-canonical S scalar -> SetCanonicalBytes error path
		sig := append(make([]byte, 32), nonCanonical...)
		if verifyCardano(identity, msg, sig) {
			t.Error("non-canonical scalar should fail")
		}
	})

	t.Run("ed25519-blake2b", func(t *testing.T) {
		if verifyEd25519Blake2b(make([]byte, 31), msg, make([]byte, 64)) {
			t.Error("short pubkey should fail")
		}
		if verifyEd25519Blake2b(nonCanonical, msg, make([]byte, 64)) {
			t.Error("non-canonical point should fail")
		}
		sig := append(make([]byte, 32), nonCanonical...)
		if verifyEd25519Blake2b(identity, msg, sig) {
			t.Error("non-canonical scalar should fail")
		}
	})

	t.Run("curve25519", func(t *testing.T) {
		if verifyCurve25519(make([]byte, 31), msg, make([]byte, 64)) {
			t.Error("short pubkey should fail")
		}
		if verifyCurve25519(make([]byte, 32), msg, make([]byte, 64)) {
			t.Error("zero point / tampered sig should fail")
		}
	})

	t.Run("starkex", func(t *testing.T) {
		if verifyStarkex(make([]byte, 31), msg, &Signature{R: make([]byte, 32), S: make([]byte, 32)}) {
			t.Error("short pubkey should fail")
		}
		if verifyStarkex(make([]byte, 32), msg, nil) {
			t.Error("nil signature should fail")
		}
		// r == 0 (out of range)
		if verifyStarkex(make([]byte, 32), msg, &Signature{R: make([]byte, 32), S: make([]byte, 32)}) {
			t.Error("zero r should fail")
		}
		// valid r, s == 0 (out of range)
		r := make([]byte, 32)
		r[31] = 0x01
		if verifyStarkex(make([]byte, 32), msg, &Signature{R: r, S: make([]byte, 32)}) {
			t.Error("zero s should fail")
		}
	})
}
