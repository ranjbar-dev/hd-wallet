...
var legacyAltcoins = []Symbol{QTUM, RVN, FIRO, MONA, PIVX}

// segwitAltcoins are the native-SegWit altcoins wired in this gap.
var segwitAltcoins = []Symbol{DGB, SYS, VIA, STRAX}
...
func TestSignTxLegacyAltcoinsOracle(t *testing.T) {
	for _, symbol := range legacyAltcoins {
		...
		t.Run(string(symbol), func(t *testing.T) {
			...
			want := legacyOracleResign(t, w, symbol, out.Encoded, utxoScript)
...
func TestSignTxSegwitAltcoinsOracle(t *testing.T) {
	for _, symbol := range segwitAltcoins {
		symbol := symbol
		t.Run(string(symbol), func(t *testing.T) {
			w, err := FromMnemonic(canonicalMnemonic)
			if err != nil {
			...
			want, msg := segwitOracleResign(t, w, symbol, out.Encoded, in.Utxo)
			if out.EncodedHex != want {
				t.Fatalf("%s tx hex mismatch\n got: %s\nwant: %s", symbol, out.EncodedHex, want)
			}
...
func legacyOracleResign(t *testing.T, w *HDWallet, symbol Symbol, encoded, utxoScript []byte) string {
	t.Helper()
	var msg wire.MsgTx
	if err := msg.Deserialize(bytes.NewReader(encoded)); err != nil {
		t.Fatalf("wire deserialize: %v", err)
	}
	priv := altcoinWalletKey(t, w, symbol)
...
func segwitOracleResign(t *testing.T, w *HDWallet, symbol Symbol, encoded []byte, utxos []*txbtc.UnspentTransaction) (string, *wire.MsgTx) {
	t.Helper()
	var msg wire.MsgTx
	if err := msg.Deserialize(bytes.NewReader(encoded)); err != nil {
		t.Fatalf("wire deserialize: %v", err)
...
// altcoinWalletKey extracts the (symbol,0) secp256k1 private key for the oracle.
func altcoinWalletKey(t *testing.T, w *HDWallet, symbol Symbol) *btcec.PrivateKey {
	t.Helper()
...
func TestUtxoAltcoinOutputDecode(t *testing.T) {
	...
	all := append(append([]Symbol{}, legacyAltcoins...), segwitAltcoins...)
	for _, symbol := range all {
		pub, err := w.PublicKeyIndex(symbol, 0)
		if err != nil {
			t.Fatalf("PublicKeyIndex(%s): %v", symbol, err)
		}
		addr, err := w.AddressIndex(symbol, 0)
		if err != nil {
		...
		var want []byte
		if _, segwit := btcAddrParams[symbol]; segwit {
...
func TestUtxoAltcoinP2SHDecode(t *testing.T) {
	...
	for _, symbol := range legacyAltcoins {
		p := utxoOutParams[symbol]
		addr := base58CheckEncode(base58BTC, p.p2shVer, scriptHash)
		got, err := bitcoinDecodeScript(symbol, addr)
		if err != nil {
			t.Fatalf("bitcoinDecodeScript(%s P2SH, %q): %v", symbol, addr, err)
		}
		if !bytes.Equal(got, wantScript) {
			t.Fatalf("%s P2SH decode(%q) = %x, want %x", symbol, addr, got, wantScript)
		}
	}

	// Native-SegWit coins: build the P2SH address from btcAddrParams.
	for _, symbol := range segwitAltcoins {
		p := btcAddrParams[symbol]
...
func TestUtxoAltcoinTxVersion(t *testing.T) {
	for _, symbol := range segwitAltcoins {
		if v := btcTxVersion(symbol); v != 2 {
			t.Errorf("btcTxVersion(%s) = %d, want 2", symbol, v)
		}
	}
	for _, symbol := range legacyAltcoins {
